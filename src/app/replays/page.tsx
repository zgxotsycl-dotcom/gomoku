...
const ReplaysPage = () => {
  ...
  const fetchReplays = useCallback(async () => {
    ...
    const { data, error } = await supabase
      .from('user_replays')
      .select(`id, game_id, is_starred, created_at, games ( winner_player, game_type )`)
      .eq('user_id', user.id)
    ...
    <div className="min-h-screen bg-gray-800 text-white p-4 sm:p-8">
      <div className="max-w-4xl mx-auto">
        <div className="flex justify-between items-center mb-6">
            ...
            <Link href="/" className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">Back to Game</Link>
        </div>
            ...
            {replays.length > 0 ? replays.map((replay) => (
              <li key={replay.id} className="p-4 bg-gray-800 rounded-md flex justify-between items-center">
                <div>
                  ...
                  <p className="text-sm text-gray-400">Mode: {replay.games?.game_type} | Winner: {replay.games?.winner_player}</p>
                  <p className="text-xs text-gray-500">Saved: {new Date(replay.created_at).toLocaleString()}</p>
                </div>
                <div className="flex items-center gap-4">
                  <button onClick={() => toggleStar(replay.id, replay.is_starred)} className="text-2xl">{replay.is_starred ? <span className="text-yellow-400">★</span> : <span className="text-gray-500">☆</span>}</button>
                  <Link href={`/replay?gameId=${replay.game_id}`} className="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700">Watch</Link>
                </div>
              </li>
            )) : (
...
// Need to import useAuth to make this work
