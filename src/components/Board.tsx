...
const PostGameManager = ({ isPlayer, isSpectator, resetGame, gameMode, room, socketRef }) => {
...
const OnlineMultiplayerMenu = ({ setGameMode, socketRef, userProfile }) => {
    const { t } = useTranslation();
...
const ReplayControls = ({ moveCount, currentMove, setCurrentMove, isPlaying, setIsPlaying, onWhatIf }) => {
    const { t } = useTranslation();
    const handleSliderChange = (e) => setCurrentMove(Number(e.target.value));
    return (
...
const PlayerDisplay = ({ profile, lastEmoticon }) => {
...
const Board = ({ spectateRoomId = null, replayGame = null }) => {
  const { t } = useTranslation();
    ...
    <div className="flex flex-col items-center">
      {gameState !== 'replay' && (
        <div className="mb-4 flex gap-4">
            <button onClick={() => resetGame('pvp')} className={`px-4 py-2 text-white rounded ${gameMode === 'pvp' ? 'bg-blue-600' : 'bg-blue-400'}`}>{t('PvsPlayer')}</button>
            <button onClick={() => resetGame('pva')} className={`px-4 py-2 text-white rounded ${gameMode === 'pva' ? 'bg-green-600' : 'bg-green-400'}`}>{t('PvsAI')}</button>
            <button onClick={() => resetGame('pvo')} className={`px-4 py-2 text-white rounded ${gameMode === 'pvo' ? 'bg-purple-600' : 'bg-purple-400'}`}>{t('PvsOnline')}</button>
        </div>
      )}
      ...
      {(gameMode !== 'pvo' || room) && (
        <div onClick={handleBoardClick} className={`bg-yellow-800 p-2 shadow-lg relative rounded-md ${isSpectator || (gameMode === 'pva' && currentPlayer === 'white') || (gameState !== 'playing' && !whatIfState) ? 'cursor-not-allowed' : 'cursor-pointer'}`} style={{ width: '64vmin', height: '64vmin' }}>
          ...
          <div className="absolute top-0 left-0 w-full h-full pointer-events-none">
            {currentBoard.map((row, r_idx) => row.map((cell, c_idx) => {
              if (cell) {
                ...
                return <div key={`${r_idx}-${c_idx}`} className={`absolute rounded-full shadow-md`} style={{ top: `calc(${(r_idx / (BOARD_SIZE - 1)) * 100}% - (${stoneSize} / 2))`, left: `calc(${(c_idx / (BOARD_SIZE - 1)) * 100}% - (${stoneSize} / 2))`, width: stoneSize, height: stoneSize, backgroundColor: cell, border: '1px solid gray' }}></div>;
              }
              return null;
            }))}
          </div>
