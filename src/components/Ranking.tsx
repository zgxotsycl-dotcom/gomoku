...
const Ranking = forwardRef<HTMLDivElement, Record<string, never>>((_props, ref) => {
  ...
  useEffect(() => {
    const fetchRankingData = async () => {
      ...
      if (!gameError && gameData) {
        ...
        gameData.forEach((g: any) => {
          if (g.player1_id) gameMap.set(g.player1_id, g.room_id);
          if (g.player2_id) gameMap.set(g.player2_id, g.room_id);
        });
    ...
    const scheduleFetch = () => {
      ...
      debounceRef.current = setTimeout(() => { void fetchRankingData(); }, 500);
    };
    const ch = supabase
      ...
      {loading ? (
        ...
        <>
          <div className="flex items-center justify-between mb-3 gap-2">
            <button onClick={() => setShowOnlyActive(v => !v)} className="px-3 py-1 rounded text-xs bg-gray-700 text-white hover:bg-gray-600">
              {showOnlyActive ? t('AllPlayers', 'All players') : t('ShowOnlyActive', 'Show only active')}
            </button>
            <button onClick={() => setSupportersOnly(v => !v)} className="px-3 py-1 rounded text-xs bg-gray-700 text-white hover:bg-gray-600">
          ...
          <div className="flex items-center justify-between mb-3 gap-2">
            ...
            />
            <button onClick={() => setSearchTerm('')} className="px-2 py-1 text-xs bg-gray-700 text-white rounded hover:bg-gray-600">
              {t('Clear', 'Clear')}
            </button>
            <button
              onClick={() => {
          ...
          <div className="flex items-center justify-end gap-2 mb-2">
            <button onClick={spectateFirstActive} className="px-3 py-1 rounded text-xs bg-green-700 text-white hover:bg-green-600 flex items-center gap-1" title={t('SpectateActive', 'Spectate active')}>
              <FaEye /> {t('SpectateActive', 'Spectate active')}
            </button>
            <button onClick={spectateRandomActive} className="px-3 py-1 rounded text-xs bg-green-700 text-white hover:bg-green-600" title={t('SpectateRandom', 'Random')}>
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      {activeRoomId && (
                        <>
                          ...
                          <a href={`/spectate?roomId=${activeRoomId}`} target="_blank" rel="noopener noreferrer" className="p-2 bg-gray-700 rounded text-white hover:bg-gray-600" title={t('OpenInNewTab', 'Open in new tab')}>
                            <FaExternalLinkAlt />
                          </a>
                          <button onClick={() => copyLink(activeRoomId)} className="p-2 bg-gray-700 rounded text-white hover:bg-gray-600" title={t('CopyLink', 'Copy link')}>
                            <FaCopy />
                          </button>
                        </>
                      )}
                      <span className="text-lg font-bold text-white">{profile.elo_rating}</span>
                    </div>
                  </li>
                );
              })
